...
Listado.prototype.obtenerCiudades = function() {
    ...
    var ciudadesNoRepetidas = sacarRepetidos(ciudades);
...
Listado.prototype.obtenerRubros = function() {
    ...
    let rubrosFiltrados = sacarRepetidos(rubros);
...
Listado.prototype.obtenerHorarios = function() {
    ...
    arrayHorarios = this.restaurantes.map(function(restaurant){
        return restaurant.horarios
    })

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarios = [];
    arrayHorarios.forEach(function(param) {
        param.forEach(function(t) {
            horarios.push(t)
        });
    });

    var horariosNoRepetidas = sacarRepetidos(horarios);
    
    return horariosNoRepetidas.sort();
}

function sacarRepetidos(param){
  
    var nuevoArreglo = param.filter(function(elem, index, self) {
